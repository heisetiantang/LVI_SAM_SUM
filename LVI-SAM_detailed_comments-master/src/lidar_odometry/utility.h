...
public:
    ...
    vector<float> edgeNum_vector;
    vector<float> surfNum_vector;
    ...
    //feature文件数据
        std::ofstream edgeNumout;//角点
        // std::ofstream surfaceNumout;//面点
        std::ofstream rotioofedge2surfout;//角点和面点比例
        std::ofstream numsurfaceCloudScanDSout;//原始面点数量；一次滤波面点数量
    //map文件中的数据
         std::ofstream mapEdgeout; //一次滤波角点数量；参与优化角点数量
             std::ofstream  mapSurfout;//二次滤波面点数量；参与优化面点数量
                 std::ofstream  coeffSumout;//角点残差；面点残差
                    std::ofstream time_framesout;//迭代时间

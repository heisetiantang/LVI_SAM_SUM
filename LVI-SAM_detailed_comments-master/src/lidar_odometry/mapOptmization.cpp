...
public:
    ...
    //参与LM的一次迭代的   角点和面点
    int laserCloudOriCornerSize = 0;
    int  laserCloudOriSurfSize = 0 ;
    //创建容器保存30次迭代的参与角点和面点的数目
    std::vector<int >laserCloudOri_CornerSizeSum;
    std::vector<int >laserCloudOri_SurfSizeSum;

    int laserCloudOri_SizeSum=0;//当前帧下标
    //统计角点和面点的残差
    double cornerCoeff = 0;
     double surfCoeff = 0;
    //创建一个"微秒"算法时间对象
    Tools::Time::AlgoTimeMs Optimization_time; 
    Tools::Time::AlgoTimeNs  Gloabl_time;

    //置信度系数
    double confidence_e =1;
    double confidence_s =1;
    ...
    mapOptimization()
    {
        ...
        **/
        ...
        ///< 10)
        parameters.relinearizeSkip = 1;//线性化频率
        // gtsam::ISAM2 *isam;
        ...
        //回调函数计算环境复杂度因子和点云数目因子
        subTrend = nh.subscribe<trajectory_output::trend>("Trend",100,&mapOptimization::trendCallback,this, ros::TransportHints().tcpNoDelay());
  ...
  void trendCallback(const trajectory_output::trend::ConstPtr &msg)
    {
        ...
        for(int i = 0; i < 8; i++)
            {
                map_env_trend[i] = msg->env_trend[i];   
            }
        map_resolution_frames=  msg->resolution_frames;
        // map_num_clound_size=msg->num_clound_size;
        

        // cout << "map_area: " << map_area << end
        // cout << "map_changeNumber: " << map_changeNumber << endl;
        // cout << "map_environmentComplexity: " << map_environmentComplexity << endl;
        //  cout << "map_env_trend: " << map_env_trend[0]<<" "<<map_env_trend[1] << endl;
        // cout << "map_resolution_frames: " << map_resolution_frames << endl;
        ...
        if (laserCloudOri_CornerSizeSum.size()<=10 ||  laserCloudOri_SurfSizeSum.size()<=10)
        {
           //向滑窗中添加最新的元素
            ...
            //计算容器的平均值
            double  Corner_last_average = accumulate( laserCloudOri_CornerSizeSum.begin(), laserCloudOri_CornerSizeSum.end(), 0.0) / size_corner_vector_tmp;
            double Surf_last_average = accumulate( laserCloudOri_SurfSizeSum.begin(), laserCloudOri_SurfSizeSum.end(), 0.0) / size_surf_vector_tmp;
            ...
            //当前帧数目
             //从容器末尾选择10个历史元素
             //计算滑窗内的平均值
            double Corner_last_average = accumulate(laserCloudOri_Corner_last.begin(), laserCloudOri_Corner_last.end(), 0.0) / laserCloudOri_Corner_last.size();            
            double  Surf_last_average = accumulate(laserCloudOri_Surf_last.begin(), laserCloudOri_Surf_last.end(), 0.0) / laserCloudOri_Surf_last.size();
            //选取滑窗内的最大值
            ...
            if(sliding_history_max ==  sliding_history_min  ) {
             sliding_window_factor = 0;
            }
            else{
    ...
    void laserCloudInfoHandler(const lvi_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        // mapEdgeout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapEdge.txt", ofstream::out | ofstream::app);
        // mapEdgeout << "\n";   
        // mapEdgeout.close();

        // mapSurfout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapSurf.txt", ofstream::out | ofstream::app);
        // mapSurfout  << "\n"; 
        // mapSurfout.close();
        ...
        if (timeLaserInfoCur - timeLastProcessing >= mappingProcessInterval) {
            ...
            */
            ...
            time_framesout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/time_frames.txt", ofstream::out | ofstream::app);
            //通过时间计数器求得经过自适应滤波之后的时间变化
            Optimization_time.start();
            
            // 求解当前帧位姿transformTobeMapped
            scan2MapOptimization();

           	// 直接在标准输出设备中输出程序的运行时间
            // Optimization_time.printElapsed();
            auto elapsed = Optimization_time.elapsed();
	        // std::cout << elapsed << "ms" << std::endl;
            time_framesout<<elapsed<<"  ";
            Gloabl_time.start();
            ...
            */
            ...
            auto elapsed_gloabl = Gloabl_time.elapsed();
            // std::cout << elapsed_gloabl << "ns" << std::endl;
            time_framesout<<elapsed_gloabl<<endl;
            time_framesout.close();
    ...
    void downsampleCurrentScan()
    {
        ...
        mapEdgeout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapEdge.txt", ofstream::out | ofstream::app);
                // cout << "edgeNum: 0.2滤波后角点" << laserCloudCornerLastDSNum << endl;
                mapEdgeout << laserCloudCornerLastDSNum << "         ";   
                // mapEdgeout.close();
       mapSurfout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapSurf.txt", ofstream::out | ofstream::app);
    //     cout << "二次滤波后面点" << laserCloudSurfLastDSNum << endl;
        mapSurfout  <<laserCloudSurfLastDSNum<< "       "; 
    //    mapSurfout.close();
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1))
                {
                    ...
                    // 可以理解为残差的权重
                    float s =confidence_e * ( 1 - 0.9 * fabs(ld2));
                    ...
                    if (s > 0.1)
                    {
                        // 将当前点pointOri保存至laserCloudOriCornerVec
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0)
            {
                ...
                if (planeValid)
                {
                    ...
                    // 残差越大，权重越小
                    // 距离LiDAR越远，权重越大
                    float s = confidence_s *( 1 - 0.9 * fabs(pd2) / sqrt(sqrt(pointSel.x * pointSel.x
    ...
    void combineOptimizationCoeffs()
    {
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; ++i)
        {
            ...
            if (laserCloudOriCornerFlag[i] == true)
            {
                ...
                // cout << "coeffSelCornerVec[i].intensity" << coeffSelCornerVec[i].intensity << endl;
                cornerCoeff += fabs(coeffSelCornerVec[i].intensity);

            }
            
        }
        // cout << "优化角点" << laserCloudOriCornerSize << endl;

   
        // combine surf coeffs
        // 遍历局部地图所有面点
        laserCloudOriSurfSize = 0;
        surfCoeff = 0;
        for (int i = 0; i < laserCloudSurfLastDSNum; ++i)
        {
            // 只有拟合结果合理的平面
            if (laserCloudOriSurfFlag[i] == true)
            {
                ...
                laserCloudOriSurfSize ++;
                // 将带有权重信息的残差push入coeffSel
                coeffSel->push_back(coeffSelSurfVec[i]);
                surfCoeff += fabs(coeffSelSurfVec[i].intensity);
            ...
            //    cout << "优化面点" << laserCloudOriSurfSize << endl;
    ...
    {
        coeffSumout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/coeffSum.txt", ofstream::out | ofstream::app);
        // 如果当前帧是第一帧，自然没法scan-to-map，退出
        if (cloudKeyPoses3D->points.empty()){
        // mapEdgeout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapEdge.txt", ofstream::out | ofstream::app);
        // cout << "edgeNum: 0.2滤波后角点" << laserCloudCornerLastDSNum << endl;
        mapEdgeout <<laserCloudCornerLastDSNum<< endl;   
        mapEdgeout.close();
        //  mapSurfout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapSurf.txt", ofstream::out | ofstream::app);
          //     cout << "二次滤波后面点" << laserCloudSurfLastDSNum << endl;
        mapSurfout <<laserCloudSurfLastDSNum <<endl; 
         mapSurfout.close();
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            int  laserCloudOriSurfSizeSum = 0;//参与优化角点总数
            int laserCloudOriCornerSizeSum = 0;//参与优化面点总数
            double cornerCoeffSum = 0;//角点残差和
            double surfCoeffSum = 0;//面点残差和
            //    mapEdgeout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapEdge.txt", ofstream::out | ofstream::app);
            // mapSurfout.open("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapSurf.txt", ofstream::out | ofstream::app);
             int iterCountNum = 0;//迭代次数
            // 以下操作迭代计算30次
            for (int iterCount = 0; iterCount < 30; iterCount++)
            {
                ...
                */
                ...
                //统计参与优化的角点和面点数目
                laserCloudOriCornerSizeSum = laserCloudOriCornerSizeSum +laserCloudOriCornerSize;
                laserCloudOriSurfSizeSum =  laserCloudOriSurfSizeSum + laserCloudOriSurfSize;
                //统计参与优化的角点和面点残差和
                cornerCoeffSum = cornerCoeffSum + cornerCoeff;
                surfCoeffSum = surfCoeffSum + surfCoeff;
                iterCountNum = 1+ iterCount;//迭代次数
            ...
            // cout <<"cornerCoeffSum"<<double (cornerCoeffSum/iterCountNum)<<endl;
            // cout <<"surfCoeffSum"<<double(surfCoeffSum/iterCountNum)<<endl;
            // cout <<"laserCloudOriCornerSizeSum"<<double(laserCloudOriCornerSizeSum/iterCountNum)<<endl;
            // cout <<"laserCloudOriSurfSizeSum"<<double(laserCloudOriSurfSizeSum/iterCountNum)<<endl;    
            //输出 每帧角点残差和  每帧面点残差和  每帧总残差和    当前帧每个角点残差  当前帧每个面点残差
            coeffSumout<<double (cornerCoeffSum/iterCountNum)<<"   "<<double(surfCoeffSum/iterCountNum)<<"   "<<double((cornerCoeffSum/iterCountNum)+(surfCoeffSum/iterCountNum))<<" "<<double(cornerCoeffSum/laserCloudOriCornerSizeSum)<<"   "<<double(surfCoeffSum/laserCloudOriSurfSizeSum)<<endl;
            coeffSumout.close();
            mapEdgeout << laserCloudOriCornerSizeSum/iterCountNum<<endl;   
            mapEdgeout.close();
            mapSurfout   <<laserCloudOriSurfSizeSum/iterCountNum<<endl; 
            mapSurfout.close();

            // 迭代优化后，再对优化结果transformTobeMapped做一个限制
            transformUpdate();
        } 
...
int main(int argc, char** argv)
{
    ...
    std::ofstream mapEdgeout("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapEdge.txt", ofstream::trunc);
    std::ofstream mapSurfout("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/mapSurf.txt", ofstream::trunc);
    std::ofstream coeffSumout("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/coeffSum.txt", ofstream::trunc);
    std::ofstream time_framesout("/home/gjm/catkin_ws_lvi/src/LVI-SAM_detailed_comments-master/src/pointChange/mapPoint/time_frames.txt", ofstream::trunc);
